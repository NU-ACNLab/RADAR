...
def rename_partic(participant, directory): 
    ...
    #DTI
    files = glob.glob(directory + "/*" + participant + "--mb4_1k_2k_64dir_1pt5mm--s26--e1*")
    for file in files:
        print(file)
        parts = file.split(".", 1)
        new_name = "sub-" + participant + "_ses-1_run-1_dwi." + parts[1]
    ...
    for file in files:
        print(file)
        parts = file.split(".", 3)
        if(parts[2] == "nii" or parts[2] == "nii.gz"):
            new_name = "sub-" + participant + "_ses-1_run-1_T1w.nii.gz"
        else:    
            new_name = "sub-" + participant + "_ses-1_run-1_T1w." + parts[2]
        print(directory + "/" + new_name)
        os.rename(file, directory + "/" + new_name) 
...
def main():
    key = participant
    value = "/Users/katharinaseitz/Documents/dicom_conversions/radar_bids_conversion/bids/sub-" + participant + "/ses-1" 
    makedir(key, value)
