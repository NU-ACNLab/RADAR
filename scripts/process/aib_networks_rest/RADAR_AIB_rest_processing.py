### This script conducts the processing steps after fmriprep for resting state
...
def postproc_rest(sub, ses, funcindir, sesoutdir):
    ...
    if(True):    
        ...
        print(funcindir) #TODO remove this print statement
        i = 1
        while(i <= len(glob.glob(funcindir+"*ses-1_task-rest_run-0*_space-MNI152NLin6Asym_desc-preproc_bold.json"))):
            ...
            rest_cen2.to_filename(sesoutdir+'/'+sub+'_'+ses+'_task-rest_run-0' + str(i) + '_ffd-1_final.nii.gz')
            i = i + 1
            
            ### UNCOMMENT BELOW IF RUNNING SCRIPT IN CHUNKS
...
def main():
    ...
    indir = '/projects/b1108/studies/RADAR/data/preprocessed/neuroimaging/fmriprep_nofmap/'
    outdir = '/projects/b1108/studies/RADAR/data/processed/neuroimaging/AIB_RestOnly_Networks/'
    subjects = os.scandir(indir)
    tr_counts = [["ID", "ses", "run", "cleaned_shape"]]
    #subjects = glob.glob('/projects/b1108/studies/RADAR/data/processed/neuroimaging/ses-1_v23_2_0_nofmap/sub-t102*')
    for sub in subjects:
